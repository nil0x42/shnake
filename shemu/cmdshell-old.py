...
class Cmd(cmd.Cmd):
    ...
    def __init__(self, unherit=None, completekey='tab',
                 stdin=None, stdout=None, stderr=None):
        """Initialize cmdshell instance

        Considering command recursivity with dedicated file descriptors,
        each instruction block is processed through a Cmd() sub instance
        that unherits form the caller's self object.
        Basically, the main Cmd() instance must be initialized leaving
        the `unherit` argument as it is in order to process shell building.
        Then, the internal command runner calls another Cmd() instance that
        just unherits from the calling one with this aproximate syntax:
        >>> Cmd(self)(instructionList)

        """
        # if unherit, bypass standard initialization:
        if unherit.__class__ is self.__class__:
            self = unherit
            return
...
class StringLexer:
    ...
    def __init__(self):
        ...
        redirector = (fd_redir | full_redir | here_doc | add_to_file |
                      fd_bind).setParseAction(lambda token: tuple(token))
